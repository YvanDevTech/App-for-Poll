...
cipher = AES.new(mbakop_polls.settings.SECRET_KEY.encode('utf-8'), AES.MODE_ECB)
...
class MyPollAppUserManager(PolymorphicManager,BaseUserManager):
...
class MyPollAppUser(User, AbstractBaseUser, PermissionsMixin):
    objects = MyPollAppUserManager()
...
from polls.models_ import VotingPoll
...
class MyPollAppUserAnonymous(UserAnonymous):
