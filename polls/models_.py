...
from accounts.models import MyPollAppUser, User
...
class Poll(PolymorphicModel):
    ...
    admin = models.ForeignKey(MyPollAppUser, on_delete=models.CASCADE, related_name='polls')
...
class VotingPoll(Poll):
    ...
    option_choice = models.BooleanField(default=True, verbose_name=_("A voter can leave candidates not ranked."))
    option_modify = models.BooleanField(default=True, verbose_name=_("The administrator (that is, you) can add or "
                                                                     "remove candidates whenever he / she wants."))
    option_shuffle = models.BooleanField(default=False, verbose_name=_("The winner will be chosen randomly "
                                                                       "(if there is an equality)."))
    winner_poll = models.PositiveIntegerField(default=1, validators=[
            MaxValueValidator(100),
            MinValueValidator(1)
        ],  verbose_name=_("Number of Winner"))  # new one !!!!!!!!!!!!!!
    ...
    def voting_profile(self):
        ...
        if nb_candidates:
            iterator = iter(VotingScore.objects.filter(candidate__poll=self)\
                            .values('voter__id', 'voter__nickname', 'voter__polymorphic_ctype', 'value',
                                    'candidate__id') \
                            .order_by('last_modification', 'candidate'))

            content_type = ContentType.objects.get_for_model(MyPollAppUser).id
            ...
            while not finished:
                try:
                    ...
                    yield {
                        ...
                        'scores': scores,
                        'mbakopuser': current['voter__polymorphic_ctype'] == content_type
                    }
